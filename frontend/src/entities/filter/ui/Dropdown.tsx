...
export function DropDown() {
    ...
    const filteredSecondDropdown = selectedFirstDropdown === firstDropdown[0] ? [secondDropdown[0], secondDropdown[2]] : [secondDropdown[1], secondDropdown[3]];

    return (
    <div className="space-x-4 mb-8">
        <DropdownMenu options={firstDropdown} selectedOption={selectedFirstDropdown} setSelectedOption={handleFirstDropdownChange} />
        <DropdownMenu options={filteredSecondDropdown} selectedOption={selectedSecondDropdown} setSelectedOption={setSelectedSecondDropdown} />
    </div>
    );
}
...
function DropdownMenu({ options, selectedOption, setSelectedOption }: DropdownMenuProps) {
    ...
    <Menu as="div" className="relative inline-block text-left font-['Pretendard-Thin']">
        ...
        <Menu.Button className="w-[10rem] inline-flex justify-between rounded-md bg-[#111111] px-4 py-2 text-sm text-[#888888] shadow-sm ring-1 ring-inset ring-[#444444] hover:bg-[#242628] hover:text-[#cccccc]">
            {selectedOption}
            <img src="./src/entities/filter/image/Icon-Dropdown.png" className="w-[0.625rem] h-[0.375rem] self-center ml-auto" alt="Dropdown Icon" />
        </Menu.Button>
        </div>
        <Transition
            as={Fragment}
