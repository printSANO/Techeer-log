...
# nginx 가 정상 동작하고 있다면 == 아예 초기 세팅 단계가 아니라면
  ...
  # frontend 를 다시 build
  docker-compose -f $COMPOSE_FILE up --build --wait -d frontend
  ...
  case $backend_state in
    ...
    # 그 후, backend1 종료
  	0)
      	docker-compose -f $COMPOSE_FILE up --build --wait -d backend2
    ...
    # 그 후, backend1 종료
    1)
      docker-compose -f $COMPOSE_FILE up --build --wait -d backend1
      reload_nginx $NGINX_CONF_1
      docker-compose -f $COMPOSE_FILE stop backend2
      ;;
